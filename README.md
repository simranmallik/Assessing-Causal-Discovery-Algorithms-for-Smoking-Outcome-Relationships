...
## Overview
This project develops a real-world testbed for evaluating the effectiveness of two causal discovery algorithms, the Peter-Clark (PC) algorithm and the Greedy Equivalence Search (GES) algorithm, in epidemiology.

Specifically, we use data from the CDC’s National Health and Nutrition Examination Survey (NHANES) to analyze established causal relationships between smoking status and various health outcomes (e.g., COPD, stroke, heart disease). The goal is to understand if these algorithms' are able to correctly identify causal structures and directionality on real-world data.

## Methodology & Data
* Data Source: National Health and Nutrition Examination Survey (**NHANES**, 2009–2018 cycles), ~28,500 individuals.
* Causal Relationships: Ground truth causal links (e.g., Smoking Status $\rightarrow$ COPD) were defined using established medical literature.
...
* Robustness: Both algorithms were tested using bootstrapping (30 iterations) and applied across demographic features (age, gender, ethnicity) to assess stability and heterogeneity.
...
* PC proved more eager to find edges and stability in subgroup analyses due to its constraint-based nature and more lenient statistical threshold.
